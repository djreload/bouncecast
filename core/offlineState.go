...
func makeVariantIndexOffline(index int, offlineInitSrcPath string, offlineSegmentSrcPath string) {
	...
	if _, err := _storage.Save(initDestPath, 0); err != nil {
		log.Warnln(err)
	}
	...
	if _, err := _storage.Save(segmentDestPath, 0); err != nil {
		log.Warnln(err)
	}
